...
function Header() {
  ...
  return (
    <div>
        ...
        <form className="w-full" action={`/search/${search}`} >
          <div className="flex items-center text-zinc-200 focus-within:text-zinc-400 transition-all duration-500">
            ...
            <input
              ...
              className="max-w-xs placeholder-zinc-300 w-full rounded-lg pr-3 pl-10 py-2 font-semibold bg-zinc-800 bg-opacity-[0.25] text-zinc-200 border-none focus:outline-none transition-all duration-500 outline-zinc-200"
              placeholder="Search"
            />
          </div>
        </form>
      ...
      <hr className="border-t-[0.1px] border-zinc-400 border-opacity-80 w-5/6"/>
      </div>
    </div>
  );
}
