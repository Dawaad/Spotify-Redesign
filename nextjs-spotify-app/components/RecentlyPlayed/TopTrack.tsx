...
function TopTrack({
  ...
  return (
    <div onClick={playSong} className="h-[3.55rem] rounded-lg grid grid-cols-4 md:grid-cols-2 lg:grid-cols-4 bg-zinc-300  text-zinc-800 shadow-md shadow-zinc-500 hover:bg-opacity-[0.85]">
      <div className="col-span-2 flex items-center space-x-6 md:space-x-3 lg:space-x-6 ml-4 ">
        ...
        />
        <div className="w-64 truncate font-bold text-zinc-800 text-sm ">{track.name}</div>
      </div>
      ...
      <div className="font-semibold flex md:hidden lg:flex items-center text-sm ">{convertMilliseconds(track.duration_ms)}</div>
